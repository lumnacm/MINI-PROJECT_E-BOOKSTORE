...
def admin_view_booking(request):
    ...
    a=Order.objects.filter(Q(status='ordered') | Q(status="paid"))
...
def view_cart(request):
    ...
    return render(request, 'user/view cart.html', {'cart_data': cart_data, 'total_cart_price': total_cart_price})
...
def view_saled_book(request):
    saled_book = Bill_details.objects.filter(ORDER__status='paid')
